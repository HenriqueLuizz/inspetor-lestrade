...
def config():
    ...
    ipset.setAppserverPath(path)
    ipset.setConfig('appserver_path',path)
    ...
    ipset.setAppserverName(name)
    ipset.setConfig('appserver_name',name)

    ips = ipset.getIniConns()
    ipset.setConns(ips)
    ipset.setConfig('conns',ips)
    ...
    if click.confirm('Deseja configurar agora a lista de ips que o PROTHEUS CLI ' + click.style('SEMPRE',bold=True) +' deixará ativo?'):
        ...
        if click.confirm('Confirma a gravação dos IPs acima?'):
            ipset.setAlwaysup(ipsativo)
            ipset.setConfig('alwaysup',ipsativo)           
        else:
